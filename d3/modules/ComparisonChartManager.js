class ComparisonChartManager {
    ...
    updateDimensions() {
        ...
        this.height = Math.max(
            0,
            fullHeight - this.margin.top - this.margin.bottom
        );
    ...
    render() {
        ...
        if (!variable) {
            this.renderMessage(
                "Selecciona un agente contaminante para visualizar la comparación."
            );
            this.clearChart();
            return;
        }
        ...
        let filtered = this.data.filter((d) => d.variable === variable);

        if (this.activeStations && this.activeStations.length > 0) {
        filtered = filtered.filter(d => {
            const station = d.station ? d.station.toString() : '';
            return this.activeStations.includes(station);
        });
    }

        if (!filtered.length) {
            this.renderMessage(
                "No hay mediciones para el contaminante y estaciones seleccionadas."
            );
            this.clearChart();
            return;
        }
        ...
        const allValues = aggregated.flatMap((series) =>
        ...
        this.xAxisLabel.attr("x", this.width / 2).text("Mes");
        ...
        points
            ...
            .on("mouseenter", (event, d) =>
                this.showTooltip(event, d, this.currentVariable)
            ) // ← CORREGIDO
    ...
    renderLegend() {
        ...
        const startY =
            this.margin.top + Math.max(0, (this.height - totalHeight) / 2);
        ...
        merged.select("rect").attr("fill", (d) => this.colors[d] || "#999");
    ...
    showTooltip(event, dataPoint, variable) {
        ...
        this.tooltip.style("opacity", 1).html(`
                <strong>${this.currentVariable}</strong><br>
